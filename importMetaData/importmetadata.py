...
def extract_metadata_cli(path, rel_path, stat, start_time):
    try:
        ...
        video_tracks, audio_tracks, text_tracks, chapters = [], [], [], []
        ...
        for track in tracks:
            ...
            if ttype == 'General':
                ...
                if "Menu" in [t.get('@type') for t in tracks]:
                    for t in tracks:
                        if t.get('@type') == "Menu" and "extra" in t:
                            row["Chapters"] = json.dumps(t["extra"])

            elif ttype == 'Video':
                video_tracks.append({
                    k: v for k, v in track.items()
                    if k in ("@type", "StreamOrder", "ID", "Format", "Format_Level",
                ...
                audio_tracks.append({
                    k: v for k, v in track.items()
                    if k in ("@type", "@typeorder", "StreamOrder", "ID", "Format",
                ...
                text_tracks.append({
                    k: v for k, v in track.items()
                    if k in ("@type", "@typeorder", "StreamOrder", "ID", "Format", "CodecID",
                             "Language", "Default", "Forced")
                })

        row["Audio Channels"] = '+'.join(channel_counts)
        row["Audio Tracks"] = '; '.join(details)
        row["Video Tracks"] = json.dumps(video_tracks)
        row["Audio Tracks JSON"] = json.dumps(audio_tracks)
        row["Sub Titles"] = json.dumps(text_tracks)

        row["Video Track Titles"] = ", ".join([t.get("Title", "") for t in tracks if t.get("@type") == "Video" and "Title" in t])
        row["Audio Track Titles"] = ", ".join([t.get("Title", "") for t in tracks if t.get("@type") == "Audio" and "Title" in t])
